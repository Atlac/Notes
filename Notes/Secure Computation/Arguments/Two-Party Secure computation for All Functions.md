...
#### How it works:
1. **Key Generation:** The Receiver generates two public encryption keys ($pk_0$​ and $pk_1$​). However, they only create a valid secret decryption key for the public key that corresponds to their choice. The other public key is sampled randomly so that it looks valid, but the Receiver has no way to actually decrypt messages sent to it.
...
##### Against the Sender:
The sender must not learn absolutely nothing about the receiver's selection of the bit.

__Real World:__  In the protocol the receiver generate one pair of public and private key ($pk_b,sk_b$) using the generation algorithm ($GEN$) for the chosen bit, and generate a "dummy" $pk_{1-b}$ using a sampling algorithm ($Samp$) for the unchosen bit.

__The Simulation:__ To simulate the sender's view without knowing the receiver's bit we can simply run the $GEN$ algorithm 2 times since the keys generated by the $GEN$ and $Samp$ algorithm are __identically distributed__ the two type of keys, the sender simulated view cannot be distinguished from the real view, this means that is not possible to distinguish the chosen bit from the unchosen.

##### Security Against the Receiver:
The receiver must only learn their chosen message ($x_b$) and learn absolutely nothing about the unchosen message ($x_{1-b}$).

__Real World:__  In the protocol the receiver generate one pair of public and private key ($pk_b,sk_b$) using the generation algorithm ($GEN$) for the chosen bit, and generate a "dummy" $pk_{1-b}$ using a sampling algorithm ($Samp$) for the unchosen bit.

__The Simulation:__  The simulator only has access to the receiver's chosen bit $b$ and the corresponding message $x_b$ using $pk_{b}$ to create the cipher text $c_b$, while for the unchosen message encrypts a dummy string of zeroes using $pk_{1-b}$ to create the second cipher text $c_{1-b}$.
Since the Receiver only has the $sk_{b}$ can only decipher the information about the cipher-text $c_b$  and both the cipher-text are indistinguishable computationally.